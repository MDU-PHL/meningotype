...
class Verification:
    ...
    def get_sed_cmd(self):
        ...
        # Lint error: W605 invalid escape sequence '\.'
        sed_cmd = f"| sed 's/{first_sed}//g' | " + r"sed 's/\.fa//g'"
    ...
    def get_original(self):
        ...
        if self.original_results.exists():
            tab = pandas.read_csv(self.original_results, sep='\t', index_col=False, usecols=[
                                                                                            'Dummy ID',
                                                                                            'expected_serotype',
                                                                                            'expected_PorA',
                                                                                            'expected_FetA']
            ...
            print(f"""The original results seem to be missing.
                    File {self.original_results} can not be found.
                    Please make sure you have a file for comparison.
                """)
            raise SystemExit
